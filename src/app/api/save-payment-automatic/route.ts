...
export async function POST(req: NextRequest) {
  ...
  try {
    const { sessionId, email, plan, duration, promoCode, discountPercentage } = await req.json();
    
    console.log("📧 Payment details:", { sessionId, email, plan, duration, promoCode, discountPercentage });
    ...
    const periodEnd = new Date(currentDate.getTime() + 30 * 24 * 60 * 60 * 1000);
    
    // Calculate amount with discount based on plan and duration
    let originalAmount = 99.00; // Default monthly price
    
    // Set proper amount based on plan and duration
    if (plan === 'student_plus' || plan === 'teacher_plus') {
      if (duration === 'yearly') {
        originalAmount = 999.00; // Yearly price
      } else {
        originalAmount = 99.00; // Monthly price
      }
    ...
    if (discountPercentage && discountPercentage > 0) {
      ...
      console.log(`💰 Price calculation: Original=${originalAmount.toFixed(2)}, Discount=${discountPercentage}%, Applied=${discountApplied.toFixed(2)}, Final=${finalAmount.toFixed(2)}`);
    } else if (promoCode) {
      ...
      finalAmount = originalAmount - discountApplied;
      console.log(`💰 Price calculation with promo code: Original=${originalAmount.toFixed(2)}, Discount=${finalDiscountPercentage}%, Applied=${discountApplied.toFixed(2)}, Final=${finalAmount.toFixed(2)}`);
    }
    ...
    console.log("💰 Discount Percentage:", finalDiscountPercentage || "None", "%");
