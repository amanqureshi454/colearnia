...
export async function GET(
  ...
  try {
    ...
    console.log("🔍 Checking session metadata for promo code:", session.metadata);
    if (session.metadata?.promoCode) {
      promoCode = session.metadata.promoCode;
      discountPercentage = session.metadata.discountPercentage ? parseFloat(session.metadata.discountPercentage) : null;
      console.log("🎫 Found promo code in metadata:", promoCode);
    }
    
    // Check if discount was applied by comparing amounts
    if (session.amount_total && session.amount_subtotal && session.amount_subtotal > session.amount_total) {
      ...
      if (!discountPercentage && session.amount_subtotal > 0) {
        discountPercentage = Math.round(((session.amount_subtotal - session.amount_total) / session.amount_subtotal) * 100);
      }
      
      // If no promo code from metadata, try to get from Stripe promotion codes
      if (!promoCode && session.total_details?.breakdown?.discounts && session.total_details.breakdown.discounts.length > 0) {
        const discount = session.total_details.breakdown.discounts[0];
        console.log("🎫 Discount applied in Stripe:", discount);
        console.log("🎫 Discount object keys:", Object.keys(discount));
        console.log("🎫 Discount promotion_code:", (discount as any).promotion_code);
        
        // Get promotion code details
        if ((discount as any).promotion_code) {
          try {
            console.log("🎫 Attempting to retrieve promotion code:", (discount as any).promotion_code);
        ...
        console.log("🎫 No discounts found in session or promo code already exists");
      }
    ...
    if (discountPercentage && discountPercentage > 0 && !promoCode) {
      ...
      console.log("🎫 Setting default promo code for detected discount:", promoCode);
    }
